...
function export(itemName)--
    old = meExport.getItemsInNetwork({name = itemName})
    meExport.setExportConfiguration(exportSide, 1, {name = itemName})
    os.sleep(.5)
    meExport.setExportConfiguration(exportSide, 1, nil)
    new = meExport.getItemsInNetwork({name = itemName})
    while #old == #new do
        new = meExport.getItemsInNetwork({name = itemName})
    end-- busy wait until we actually see the item in me
end
...
function check_and_replace()
  for i, slot in ipairs(slot_order) do
    ...
    if item then
      ...
      elseif item.name == "gregtech:gt.360k_NaK_Coolantcell" and item.damage/item.maxDamage <= .15 then
